...
function registerSettingsCommands(program) {
  ...
  program
    ...
    .action((value) => {
      if (value === undefined) {
        const cfg = loadConfig();
        console.log(cfg.cornerRadius !== undefined ? String(cfg.cornerRadius) : '0 (default, from pack)');
        return;
      }
      const n = Number(value);
      if (!Number.isFinite(n) || n < 0) {
      ...
      console.log(`Corner radius set to ${n}px. Restart peon-pet for it to take effect.`);
    });
